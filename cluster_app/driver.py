...
def fullrecursionpipeline(*args):
	 ...
	 if ranked_amount  >= snpthresh:
		...
		command = "java -classpath .:/usr/local/MATLAB/MATLAB_Runtime/v91/toolbox/javabuilder/jar/javabuilder.jar:/mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/scripts/SigClust/for_testing/SigClust.jar getClust /mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/rishub_backup/testing_brad/" + cluster_parameters[3] + " /mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/rishub_backup/testing_brad/" + cluster_parameters[4] + " " + cluster_parameters[6]
		print command
		...
		for value in sampleQueue.values() :
			...
			if value == True:
				...
				if bul_counter ==  len(sampleQueue):
					...
					print sampleQueue
					command = "python matcher.py " + finalheadings_string + " " + finalconsensus_string
					subprocess.call(command, shell = True)
