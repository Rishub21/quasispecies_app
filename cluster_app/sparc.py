...
def pipeline(sampleID, reference, ranking_method, consensus_bul, *optionalfilters): #  must put the optional filters in string format
	...
	command = "blasr " + h5file + " " + filereference + " -minMatch 19 -nproc 16 -bestn 1 -unaligned "+ unalignedfile+ " -m 5 -out " +m5file
        ...
        command = "/mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/software/Sparc-master/Sparc b "+filereference+" m "+m5file+" k 2 g 2 c 2 t 0.1 o "+sample
        print command
        print >> sys.stderr
        subprocess.call(command, shell =True)	


        command = "blasr " + h5file + " " + consensusfile + " -minMatch 19 -nproc 16 -bestn 1 -unaligned "+ unalignedfile+ " -m 5 -out " +m5file
        print command
        subprocess.call(command, shell = True)

        command = "/mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/software/Sparc-master/Sparc b "+consensusfile+" m "+m5file+" k 2 g 2 c 2 t 0.1 o "+sample
        print command
        print >> sys.stderr
        subprocess.call(command, shell =True)
	...
	command = "java -jar /mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/rishub_backup/script/VarScan.v2.4.2.jar readcounts" + " "+ pileupfile + " " + " --output-file" + " " + readcountsfile + " --min-base-qual 5"
	print command	
	print >> sys.stderr
	subprocess.call(command, shell = True)
	print >> sys.stderr, "\n readcounts_file done!\n" + str(readcountsfile)  +   "\n"
	
	command = "java -cp /mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/rishub_backup/script/ ReadCount_consensus " + readcountsfile # running as background process. This creates the consensusfile. We actually declared it before, but now it has actually been filled  
	print command
	subprocess.call(command, shell = True)
	...
	command  = "java -cp /mnt/LHRI/Bioinformatics/Projects/Quasispecies/manuscript/rishub_backup/script/ ReadCount_snp" + " " + readcountsfile
	print command	
	print >> sys.stderr 
	subprocess.call(command, shell = True)
	print >> sys.stderr, "\n snp_file is done!\n"  + str(snpfile) + "\n"
	...
	command = " python rankSNVs.py" + " " + str(snpfile)+ " " + rankedfile  + " " + ranking_method +  " "  #" " + "-s" + " " + str(args.starting_index) + " " + "-e" + " " + str(args.ending_index) + " " + "-perc" + " " + str(args.percentage) + " " + "-pval" + " " + str(args.pval_thresh) + " " + "-r "+ " " + str(args.read2_thresh) 
